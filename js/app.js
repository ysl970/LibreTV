...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤"伦理片"等成人内容';
        }
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `RTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - RTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }

        // 处理搜索结果过滤：如果启用了成人内容过滤，则过滤掉分类含有敏感内容的项目
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'RTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'RTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'RTV-Settings_' + times + '.json');
}
