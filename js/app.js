...
function playVideo(url, vod_name, sourceCode, episodeIndex = 0) {
    ...
    // 添加episodes参数
    if (window.currentEpisodes && Array.isArray(window.currentEpisodes) && window.currentEpisodes.length > 0) {
        watchUrl += `&episodes=${encodeURIComponent(JSON.stringify(window.currentEpisodes))}`;
    }
    // 添加返回URL参数
