...
function playFromHistory(url, title, episodeIndex, playbackPosition = 0) {
    try {
        ...
        if (historyRaw) {
            ...
            // 根据标题查找匹配的历史记录
            historyItem = history.find(item => item.title === title);
            ...
            if (historyItem && historyItem.episodes && Array.isArray(historyItem.episodes)) {
                ...
                if (historyItem.url) {
                    try {
                        const urlObj = new URL(historyItem.url, window.location.origin);
                        sourceCode = urlObj.searchParams.get('source_code') || '';

                        // 尝试从URL中提取视频ID
                        const videoUrl = urlObj.searchParams.get('url') || '';
                        if (videoUrl) {
                            // 从视频URL中提取ID
                            const idMatch = videoUrl.match(/\/id\/(\w+)\.html/) ||
                                           videoUrl.match(/[?&]ids=(\w+)/) ||
                                           videoUrl.match(/[?&]id=(\w+)/);
        ...
        // 如果有源代码和视频ID，尝试刷新集数数据
        if (sourceCode && videoId) {
            console.log(`尝试刷新 "${title}" 的集数数据，源: ${sourceCode}, ID: ${videoId}`);

            // 异步刷新集数，不阻塞播放
            setTimeout(() => {
                refreshEpisodesFromAPI(title, sourceCode, videoId, episodesList.length, historyItem);
            }, 1000);
        }
...
// 从API刷新集数数据
async function refreshEpisodesFromAPI(title, sourceCode, videoId, currentEpisodeCount, historyItem) {
    try {
        // 构建API参数
        let apiParams = '';
        ...
        console.log(`正在从API获取 "${title}" 的最新集数数据...`);

        // 发起API请求
        const response = await fetch(`/api/detail?id=${encodeURIComponent(videoId)}${apiParams}${cacheBuster}`);
        ...
        if (data.episodes && Array.isArray(data.episodes) && data.episodes.length > 0) {
            ...
            // 比较新旧集数数量
            if (safeEpisodes.length > currentEpisodeCount) {
                console.log(`发现新集数! 原有: ${currentEpisodeCount}, 现有: ${safeEpisodes.length}`);

                // 更新localStorage中的currentEpisodes
                localStorage.setItem('currentEpisodes', JSON.stringify(safeEpisodes));

                // 更新历史记录中的集数数据
                if (historyItem) {
                    updateHistoryEpisodes(title, safeEpisodes);
                }

                // 显示通知
                showToast(`发现新集数! 原有 ${currentEpisodeCount} 集，现有 ${safeEpisodes.length} 集`, 'success', 5000);
            } else {
                console.log(`未发现新集数，保持原有 ${currentEpisodeCount} 集`);
            }
        } else {
            console.log(`API未返回有效的集数数据`);
        }
    } catch (e) {
        console.error('刷新集数数据失败:', e);
    }
}

// 更新历史记录中的集数数据
function updateHistoryEpisodes(title, newEpisodes) {
    try {
        const historyRaw = localStorage.getItem('viewingHistory');
        ...
        if (index !== -1) {
            // 更新集数数据
            history[index].episodes = [...newEpisodes]; // 深拷贝
            console.log(`已更新历史记录中 "${title}" 的集数数据: ${newEpisodes.length}集`);

            // 保存回localStorage
            localStorage.setItem('viewingHistory', JSON.stringify(history));
        }
    } catch (e) {
        console.error('更新历史记录中的集数数据失败:', e);
    }
}
