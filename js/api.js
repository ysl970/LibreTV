...
async function handleSpecialSourceDetail(id, sourceCode) { // 
    try {
        // API_SITES[sourceCode].detail 现在是基础URL，需要拼接
...
// customApiBaseUrl 可以是 apiInfo.detail (如果配置了专门的详情页基础URL) 或 apiInfo.url (回退到API基础URL)
    ...
    try {
        ...
        return JSON.stringify({ // 
            ...
            videoInfo: { // 
                title, desc, // 
                source_name: '自定义源', // 
                source_code: 'custom' // 代表是自定义类型，具体是哪个custom由前端决定 
            }
        });
    } catch (e) {
...
async function handleApiRequest(url) { // 
    ...
    try {
        if (url.pathname === '/api/search') { // 
            ...
            // 验证API与source
            // source 参数可能是内置源的 key (如 'bfzy') 或自定义源的标识 (如 'custom_0', 'custom_1' 等)
            // customApi 参数则是在 source 是自定义源时，传递其基础URL
            ...
            try {
                ...
                result.list.forEach(item => { // 
                    // source.startsWith('custom_') ? '自定义源' : API_SITES[source].name
                    // 对于自定义源，其具体名称由 APISourceManager 管理，前端已知
                    // 这里可以简化为，如果 source 是 custom_X，则前端会处理具体名称
        ...
        if (url.pathname === '/api/detail') { // 
            ...
            try {
                ...
                // 处理需要HTML抓取的自定义源
                    ...
                    if (apiInfo) {
                        const detailScrapeUrl = apiInfo.detail || customApi; // customApi 是该自定义源的基础URL从查询参数传来
                ...
                // 标准API详情 (JSON)
                    ...
                    const detailUrl = sourceCode.startsWith('custom_') // 
                        ? `${customApi}${API_CONFIG.detail.path}${id}` // customApi 是该自定义源的基础URL 
